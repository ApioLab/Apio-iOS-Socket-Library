...
- (void)emit:(NSString *)event args:(NSDictionary *)args {
    NSError *error;
    ...
    NSString *test;
    if (! jsonData) {
        NSLog(@"[ApioIOSocket] JSON format error: %@", error.localizedDescription);
    } else {
        test = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
        test = [NSString stringWithFormat:@"'%@', '%@'", event, test];
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"%@", test);
            [self.javascriptContext evaluateScript: [NSString stringWithFormat: @"objc_socket.emit(%@);", test]];
        });
    }
}
